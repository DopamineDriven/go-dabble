...
// Box is a struct literal defining the shape of 2 sets of 3D coords -- min and max
...
func intersection(t *testing.T) {
	vectorUno := Box{T{0, 0, 0}, T{4, 7, 10}}
	vectorDos := Box{T{1, 1, 1}, T{5, 8, 11}}
	if !vectorUno.Intersects(&vectorDos) {
		t.Fail()
	}
}
